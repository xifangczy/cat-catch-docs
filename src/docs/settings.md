...
- 不清空，任何情都不清理数据，直到用户关闭当前页面的标签。
- 正常清理，用户主动刷新页面或点击跳转到新页面，当前抓取的数据会被清空。
- 更频繁，在正常清理的基础上，会更加敏感，例如切换新视频也会触发清空。

部分网站会自行多次刷新页面，导致抓取数据被清空，遇到嗅探不到资源的网站，请先尝试清理模式改为 不清理。

### 嵌套在线ffmpeg

在使用[在线ffmpeg](online-ffmpeg)过程中，各种复杂原因数据传输失败，在线ffmpeg没收到数据，可以打开这个选项，使用嵌套的在线ffmpeg解决。

## 发送数据 {#send}

popup页面点击发送按钮，能够自定义发送数据到第三方地址上，请求体 确保为JOSN格式
